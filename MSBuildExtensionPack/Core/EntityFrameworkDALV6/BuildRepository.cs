			...
			)
        {
			var _ResultFromDataSource = 
				(
					...
					select new MSBuildExtensionPack.DataSourceEntities.Build.Default { Id = t.Id, SolutionId = t.SolutionId, Name = t.Name, Description = t.Description, BuildStartTime = t.BuildStartTime, Solution_1_Name = Solution_1.Name } into vD2
					select vD2
				);
			...
			)
        {
			var _ResultFromDataSource = 
				(
					...
					group t by new { SolutionId = t.SolutionId, Name = string.Format("{0}", Solution_1.ExternalParentId) } into tG
					let _CountPerFK = tG.Count()
					select new MSBuildExtensionPack.DataSourceEntities.Build.DefaultGroupedDataView { SolutionId = tG.Key.SolutionId, CountPerFK = _CountPerFK, Name = tG.Key.Name } into vD3
					select vD3
				);
			var _retval = _ResultFromDataSource;
