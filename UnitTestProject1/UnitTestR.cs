...
namespace UnitTestProject1
{
    ...
    public class UnitTest1R
    {
        ...
        public static async Task<bool> Test1()
        {
            ...
            string urlA = "http://localhost:10812/api/BuildLogApi/Test";
            var a = new Uri(urlA);
        ...
        {
            MSBuildExtensionPack.ViewModelData.WPCommonOfBuildLogVM vm = JsonConvert.DeserializeObject<MSBuildExtensionPack.ViewModelData.WPCommonOfBuildLogVM>(Request);

            HttpClient client = new HttpClient();
            string urlA = "http://localhost:10812/api/BuildLogApi/GetWPCommonOfBuildLogVM";

            string request = JsonConvert.SerializeObject(vm);
            var httpContent = new StringContent(request, System.Text.Encoding.UTF8, "application/json");

            var response = await client.PostAsync(urlA, httpContent);

            if (response.IsSuccessStatusCode)
            {
                var result = await response.Content.ReadAsStringAsync();
                Console.WriteLine(response.Content.Headers.ContentLength);
                MSBuildExtensionPack.ViewModelData.WPCommonOfBuildLogVM newvm = JsonConvert.DeserializeObject<MSBuildExtensionPack.ViewModelData.WPCommonOfBuildLogVM>(result);
